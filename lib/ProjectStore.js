...
class ProjectStore {
  projectRef = firebase.database().ref("projects/test")
  nodes = {}
  rootId = null
  ...
  addNode(data, parentId = this.rootId) {
    ...
    this.projectRef
      ...
      .transaction(children_ids => {
        // Add the new child ID at the end of the sort order
        const entries = objectEntriesSortedByValue(children_ids)
        const largestValue = entries[entries.length - 1][1]
        return Object.fromEntries([...entries, [newKey, largestValue + 1]])
      })
  }
}
