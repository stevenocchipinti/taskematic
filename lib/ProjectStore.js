...
class ProjectNode {
  ...
  delete() {
    // TODO: This should probably be recursive to delete the whole sub-tree
...
class ProjectStore {
  ...
  deleteNode(nodeIdToDelete) {
    ...
    const updateOperationsForchildReferences = Object.values(this.nodes)
      .filter(node => node.childrenIds.includes(nodeIdToDelete))
      .reduce(
        (result, node) => ({
          ...result,
          [`nodes/${node.id}/childrenIds/${nodeIdToDelete}`]: null,
        }),
        {}
      )
  ...
  setIndex(nodeIdToMove, newIndex) {
    const parentNode = this.nodes[nodeIdToMove].parent
    this.projectRef
      .child(`nodes/${parentNode.id}/childrenIds`)
      .transaction(childrenIdsObj => {
        const sortedChildrenIds = objectKeysSortedByValue(childrenIdsObj)
        const oldIndex = sortedChildrenIds.findIndex(id => id === nodeIdToMove)
        return moveElement(sortedChildrenIds, oldIndex, newIndex).reduce(
  ...
  move(nodeIdToMove, newParentNodeId, newIndex) {
    console.log(
      `Move ${nodeIdToMove} to ${newParentNodeId} at index ${newIndex}`
    )
  }
}
