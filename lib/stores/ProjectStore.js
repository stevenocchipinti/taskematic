...
class ProjectStore {
  ...
  // getProjects() {
  //   if (!this.rootStore.userStore.ready) return undefined
  //   const uid = rootStore.userStore.user.uid
  //   firebase
  //     .database()
  //     .ref(`/users/${uid}/projects`)
  //     .on("value", snapshot => {
  //       const { data } = snapshot
  //       runInAction(() => this.projects.push(data.key))
  //     })
  // }
  ...
  async createProject(title = "New project") {
    ...
    if (!uid) throw new Error("Must be logged in to create a new project")
